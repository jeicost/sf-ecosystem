...
| `clients/discoolver/briefing` | discoolver-briefing | `prj_leUpb2tNZkSikGVeVHUt8JwJujQZ` | `team_7QGpRqqi1FjrJugGLL0sDehf` | discoolver-briefing-jeicosts-projects.vercel.app | — | Static HTML briefing page. Deployed 2026-07-19. 🪦 **RETIRADO 2026-08-20 (decisión del CEO):** el despliegue servía el material sin `noindex`, sin canonical y sin contraseña — el PDF del deck se descargaba directo (16 MB). No había fuga de datos (ni cap table, ni cuentas, ni clientes), pero era indexable por Google bajo la marca y contradecía la regla de no publicar vocabulario de pitch. `vercel remove` borró el proyecto; **la carpeta y los ficheros siguen intactos en el repo**, así que se republica con `vercel --prod` desde su carpeta. Si se republica, ponerle protección de despliegue. |
| `clients/discoolver/deliverables/investor-deck-site` | discoolver-investor-deck | `prj_clu0ci7Z7FuvEsPq6GkHzvXliP48` | `team_7QGpRqqi1FjrJugGLL0sDehf` | discoolver-investor-deck-jeicosts-projects.vercel.app | — | Static HTML + investor deck PDF. Deployed 2026-07-19. 🪦 **RETIRADO 2026-08-20 (decisión del CEO):** el despliegue servía el material sin `noindex`, sin canonical y sin contraseña — el PDF del deck se descargaba directo (16 MB). No había fuga de datos (ni cap table, ni cuentas, ni clientes), pero era indexable por Google bajo la marca y contradecía la regla de no publicar vocabulario de pitch. `vercel remove` borró el proyecto; **la carpeta y los ficheros siguen intactos en el repo**, así que se republica con `vercel --prod` desde su carpeta. Si se republica, ponerle protección de despliegue. |
| `clients/discoolver/design-studio` | discoolver-design-studio | `prj_SoMU6F5A7bvp85cfPIFimYo5B2jP` | `team_7QGpRqqi1FjrJugGLL0sDehf` | discoolver-design-studio-jeicosts-projects.vercel.app | — | Vite + React design template studio. Deployed 2026-07-19. 🪦 **RETIRADO 2026-08-20 (decisión del CEO):** llevaba roto desde su despliegue (19-jul) sin que nadie lo echara en falta: la app arrancaba, cargaba 316 KB de JS y renderizaba **cero** — `Cannot read properties of undefined (reading 'toUpperCase')` en `T14Influencers.jsx:20` y `T05ReportajePersona`. La causa no es una línea: **el sample y las plantillas no comparten contrato de datos** — `src/data/guide-config.sample.js` define `username`/`realName` y la plantilla espera `name`/`platform`/`handle`; igual en `personaDelAno`. Se retiró el despliegue (página en blanco indexable bajo la marca); **el código sigue intacto**. Para revivirlo hay que reconciliar el sample con las plantillas, no parchear los `.toUpperCase()`. |
...
| `clients/lagrimas-de-sanchez/web` | lagrimas-de-sanchez | `prj_sBGzG5owgbhBQQkrelgcjrJhM8c2` | `team_7QGpRqqi1FjrJugGLL0sDehf` | lagrimas-de-sanchez.vercel.app (dominio propio `lagrimasdesanchez.com` PENDIENTE de DNS) | — | **CREADO 2026-08-26.** Tienda D2C de la marca propia: botella serigrafiada + vino DO Vinos de Madrid. Next 16 + Tailwind 4, misma convención que `clients/salsa-burgers`. Sin git-integration: deploy con `vercel --prod` desde la carpeta. **Al crear el proyecto la protección SSO venía activada** (`all_except_custom_domains`) y devolvía 302 en todas las rutas — desactivada por API, mismo tropiezo documentado en los proyectos de Discoolver. `framework` sí se autodetectó como `nextjs` esta vez. `NEXT_PUBLIC_SITE_URL` apunta al alias de Vercel a propósito mientras el dominio no resuelva: si apuntara al dominio propio, Stripe devolvería al comprador a un sitio muerto. **La tienda NO cobra todavía**: sin `STRIPE_SECRET_KEY` el checkout devuelve 503 y la web dice "la tienda todavía no está abierta". |
