...
| `apps/sf-links` | sf-links | (not yet inspected) | — | links.startupsfactory.es | — | ⚠️ **AMBIGUOUS — DO NOT DEPLOY:** local folder is EMPTY (no code, no `.vercel`). Registry claims a domain but there is nothing to deploy. Resolve (locate/recreate the project) before any deploy from here. |
...
| `apps/mira-landing` | (unknown) | ⚠️ NOT LINKED | — | www.miralanding.com (claimed, unverified) | — | ⚠️ **AMBIGUOUS — DO NOT DEPLOY:** no `.vercel/project.json`, no confirmed Vercel project. Claims a production domain via `NEXT_PUBLIC_SITE_URL`. Risk of deploying to the wrong project (possibly onto off-limits `mira-portal`). Confirm the intended project + add its ID here before any deploy. |
