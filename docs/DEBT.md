...
**Aviso pendiente antes de activar `ENFORCE_PLAN_LIMITS` en prod**: la variable se probó y se revirtió el mismo día (ver conversación) porque el código desplegado en producción todavía no incluye este commit — activarla sin desplegar primero el código habría bloqueado Admin/Operaciones para todo cliente real con el bug ya corregido en local pero no en prod. Desplegar `b5b5142` primero, luego activar la env var.
