...
| **(e)** Light mode dependiente del parche `!important` | ~95 ficheros migrados el 2026-07-21/22. El resto y la retirada del parche se completó el 2026-07-31 — ver **(j)** ✅ |
...
## j) ✅ Retirada del parche `!important` de light mode (nueva 2026-07-22, cerrada 2026-07-31)

`app/globals.css` mantenía ~45 reglas `[data-theme="light"] ... !important` como red de seguridad tras la migración masiva a clases semánticas. La cuenta original de "33 ficheros" resultó muy inflada por el tiempo transcurrido: una auditoría por-fichero (34 agentes en paralelo, uno por fichero, cada uno leyendo el archivo completo antes de decidir) encontró que la gran mayoría ya estaba migrada de una sesión anterior no documentada aquí — solo 4 ficheros tenían clases reales pendientes (`app/login/page.tsx`, `app/(dashboard)/projects/[slug]/page.tsx`, `components/archetypes/ArchitectArchetype.tsx`, más los 2 stray-labels de la migración del chat de departamento). Verificado con grep exhaustivo app-wide tras la migración: cero ficheros restantes usando `text-white` (fuera de excepciones legítimas de texto blanco sobre fondo saturado, ~52 casos revisados uno a uno), `text-gray-*`, `bg-gray-*`, `border-gray-*`, `border-white/10` o `bg-white/3,5,8`. Borradas las reglas correspondientes de `globals.css` (bloque `TAILWIND TEXT/BG REMAPS`, ~48 líneas) — se mantiene intacto el mecanismo de selectores de atributo para inline styles (`rgba(255,255,255,...)`, paleta neón de Toolkit), que es una decisión de arquitectura aparte, no debt.
...
## xx) Auditoría de stack técnico + limpieza profunda (Fases A-D) + adopción real de `@sf/supabase` (2026-07-31)

El CEO pidió una auditoría del stack técnico de cada proyecto y de la configuración de Vercel, para decidir si valía la pena una limpieza de cara a escalar a más clientes. 3 agentes de investigación (inventario de stack, Vercel real vs. `docs/PROJECT_REGISTRY.md`, paquetes compartidos/drift de versiones) + 1 agente de diseño produjeron un plan por niveles de riesgo, ejecutado en 4 fases, 10 commits (`82bb05d`..`1056fa7`).

**Hallazgo central**: el lado de Vercel estaba sano (registro fiable, sin proyectos duplicados, Golden Rule respetada — único hueco real: `clients/lidar-home/deliverables/briefing-site/vercel.json` huérfano, sin app real detrás, ya anotado en el registro). El problema real estaba en los paquetes compartidos: 5 paquetes `@sf/*` existían para evitar duplicación, pero solo 1 tenía un consumidor real (que además nunca se despliega) — mientras `sf-cms`, `sf-crm`, `mira/portal` y (hasta que se borró por deprecated) `sf-sales-engine/portal` reimplementaban cada uno su propio cliente de Supabase.

**Fase A (limpieza segura)**: borrado `apps/mira` huérfano (config raíz sin `package.json`, la app real es `apps/mira/portal`) y `clients/salsa-burgers/web/` (solo `node_modules` muerto); borrados 7 ficheros de config duplicados en `ai-agency-sf-next` (restos de una recuperación de submódulo del 12/07, ninguna herramienta los leía); borrados `packages/auth`/`packages/ui`/`packages/config` (stubs vacíos, cero consumidores confirmado); peer-dep de React arreglado en `@sf/cms-client`; `docs/PROJECT_REGISTRY.md` corregido (2 Org ID desactualizados).

**Fase B (package managers + bug real encontrado)**: `apps/sf-sales-engine/portal` borrado entero — confirmado deprecated (su propio CLAUDE.md decía "migrado a sf-crm"), nunca desplegado (sin `.vercel/project.json`), README boilerplate nunca personalizado. **Bug real encontrado y arreglado**: `ai-agency-sf-next` no aplicaba NINGÚN estilo de Tailwind (mezcla v3/v4 — CSS con sintaxis vieja `@tailwind base/components/utilities`, plugin de postcss ya en v4) — confirmado visualmente con captura antes (HTML crudo sin diseño) / después (dashboard completo con su diseño). Corregido a v4 (`@import "tailwindcss"`, config JS eliminado, versiones fijadas). Dos conflictos de package manager resueltos: `startup-factory-web` sacado del workspace pnpm compartido (el plan original asumía mal que su `rootDirectory` de Vercel era la raíz del monorepo — verificado en vivo con `vercel project inspect` que en realidad ya usaba instalación aislada vía npm, igual que los 3 sitios de cliente ya standalone) — esto reveló de paso que el propio `package.json` raíz también necesitaba excluirla de su campo `workspaces` (npm lo lee y crasheaba al tratar de tratarla como miembro); lockfile npm huérfano de `discoolver-cms` borrado (su propio README ya decía "usa pnpm").

**Fase C (adopción real de `@sf/supabase`)**: `@sf/supabase` pasó de re-export vacío a tener 3 factories reales (`createServiceRoleClient`, `createBrowserSupabaseClient`, `createServerComponentClient` con adaptador de cookies compartido para Server Components/middleware). Adoptado en `sf-crm` (1 consumidor interno), `sf-cms` (3 wrappers + middleware + 4 imports de tipo `User`), y `mira/portal` (143 + 22 consumidores vía `lib/supabase.ts`/`lib/supabase-admin.ts`, firmas públicas sin cambios). De paso: `lib/supabase-browser.ts` de `mira/portal` borrado (duplicaba la misma lógica, cero consumidores confirmado). Bug de tipos real encontrado en el propio `@sf/supabase`: las factories basadas en `@supabase/ssr` forzaban su retorno al tipo `SupabaseClient<Database>` de `@supabase/supabase-js`, estructuralmente distinto pese a ser el mismo cliente en runtime — corregido dejando que TypeScript infiera el tipo real. **Alcance deliberadamente no ampliado**: 33+ ficheros más en `mira/portal` (rutas `app/api/**`, varios `lib/*.ts`, scripts) siguen importando `@supabase/ssr`/`@supabase/supabase-js` directamente — confirmado que es duplicación real, no solo tipos, pero migrarlos es una tarea más grande y más arriesgada (30+ rutas de producción activas) que se deja fuera de esta ronda.

**Fase D (MIRA a Next 16)**: `mira-landing` y `mira/portal` de Next 15.3.9 a 16.2.12. `mira/portal` ya tenía `proxy.ts` (lógica real) conviviendo con `middleware.ts` (shim de compatibilidad de 11 líneas) — alguien ya había preparado la migración. **Bug real encontrado antes de borrar el shim**: los `matcher` de ambos ficheros habían divergido — el de `middleware.ts` (tocado por última vez 17/07 en un fix de seguridad, "centralized middleware auth") excluía `.well-known`; el de `proxy.ts` (última tocada 28/07 por otro motivo) no. Sincronizado antes de borrar `middleware.ts`, si no `.well-known` habría empezado a pasar por el chequeo de auth completo sin motivo real.

**Verificación real, no solo build**: cada fase con `pnpm install` + `typecheck` + `build` limpios; `turbo build` final sobre las 7 apps del workspace, 7/7 exitosas. En vivo con Playwright: login real contra producción en `sf-crm` (workspace "Startup Factory", password real de `.env.local`) con sesión + dashboard completo; capturas antes/después del bug de Tailwind; en `sf-cms`, redirect de `/admin` sin sesión + render de `/admin/login` + `/api/public/pages` respondiendo 401 esperado (no 500); en `mira/portal`, rutas protegidas redirigiendo a `/login`, rutas públicas respondiendo 200, `.nft.json` del build confirmando que el fix delicado de tracing de `pdf.worker.mjs` (saga de (vv)) sigue intacto tras el upgrade — intento de login con credencial de referencia en memoria (2026-05-20) llegó correctamente a Supabase y fue rechazado por credenciales (probablemente rotadas desde entonces), no se pudo verificar una sesión completa autenticada por esto.

**Pendiente real que queda**:
- ✅ Resuelto 2026-07-31: los 33 ficheros restantes de `mira/portal` migrados a `@sf/supabase` — ver entrada **(zz)**.
- ✅ Resuelto (ya no aplicaba, 2026-07-31): `@ai-sdk/anthropic`/`ai` no existen en `apps/ai-agency-sf-next/package.json` ni se importan en ningún fichero del código — o nunca se fijaron porque ya se habían retirado antes del pineo original, o la entrada estaba desactualizada. Nada que hacer.
- ✅ Resuelto 2026-07-31: la credencial de `jacostech@gmail.com` (`reference_mira_portal_dev_credentials_access.md`) en efecto había cambiado de usuario (ID de Supabase distinto al anotado) — contraseña reseteada vía Admin API a un valor conocido y sesión completa verificada en vivo (login real + chat con streaming en las 5 páginas de departamento). Memoria actualizada.
- Decisión de negocio pendiente, no técnica: migrar `apps/startup-factory-web` a SF-CMS como el resto de landings (hoy usa `content/` local) — ya anotado en `SCHEMA.md` de memoria, no parte de este lote.

---

## yy) Repaso de UI: chat de departamento embebido + limpieza de light mode (2026-07-31)

Pedido del CEO: sacar el chat de departamento de su página aparte a la página principal de cada departamento (con las Quick Actions dentro, sin cambiar el resto del diseño), más un repaso general de sidebar/modo claro. Ejecutado con 2 workflows (34 agentes de migración por-fichero + 1 de sidebar; luego verificación en vivo + revisión adversarial de cada uno).

**Chat de departamento embebido**: nuevo `components/department-chat-panel.tsx` sustituye a `DepartmentQuickActions` en las 5 páginas (roster/comercial/strategy/operations/finanzas) — chat de equipo (reutiliza el mismo `useAgentChat`/rol virtual `dept:<slug>` que ya existía) embebido directamente, con el grid de Quick Actions original sin modificar justo debajo, en la misma sección. Botón "Talk to the whole team" y ruta `/agent/dept/[slug]` retirados (inalcanzables, cero referencias restantes confirmado). Verificado en vivo: login real, mensaje real enviado y respondido en las 5 páginas, quick actions intactas. Bug real encontrado y arreglado durante la verificación: `/operations` y `/finanzas` se quedaron con una etiqueta "ACCIONES RÁPIDAS" huérfana encima del chat (resto de la estructura anterior) — corregido.

**Limpieza del parche `!important` de light mode**: la cuenta de "33 ficheros" de la entrada (j) estaba muy inflada por el tiempo transcurrido — 34 agentes en paralelo (uno por fichero, lectura completa antes de decidir) encontraron que casi todos ya estaban migrados de una sesión anterior no documentada; solo 3 ficheros tenían clases reales pendientes. Verificado con grep exhaustivo app-wide (no solo los 34): cero ficheros restantes con `text-gray-*`/`bg-gray-*`/`border-gray-*`/`border-white/10`/`bg-white/3,5,8`; los ~52 `text-white` restantes son todos excepciones legítimas (texto blanco sobre botón/badge de color saturado). Bloque de ~48 líneas borrado de `globals.css` (se mantiene intacto el mecanismo de selectores de atributo para inline styles, que es una decisión de arquitectura aparte, no debt).

**2 bugs reales encontrados por la verificación en vivo, ambos arreglados**:
1. **Introducido por esta misma migración**: el botón "Archivar" de `projects/[slug]/page.tsx` cambió `disabled:bg-gray-700` → `disabled:bg-card` sin también adaptar el `text-white` incondicional — en modo claro, `bg-card` es blanco sólido → texto "Archivado" invisible (blanco sobre blanco). Corregido añadiendo `disabled:text-ink-secondary`, mismo patrón que `DriveFoldersPanel.tsx`.
2. **Preexistente, no introducido hoy, pero severo y fácil de reproducir**: `/login` nunca llama a `initTheme()` (tiene su propio diseño siempre-oscuro con colores literales) — si el usuario activa modo claro dentro del dashboard y luego cierra sesión (navegación client-side que no resetea `<html>`), `data-theme="light"` persiste sobre `/login` y el mecanismo de selectores de atributo recolorea a oscuro-sobre-oscuro el logo móvil, el heading, el subtítulo y el link de soporte — prácticamente invisibles. Corregido forzando `data-theme="dark"` en un `useEffect` de montaje de `/login`, sin tocar `localStorage` (la preferencia real del usuario se restaura sola al volver a entrar, vía `initTheme()` en el layout del dashboard).

**Repaso de sidebar (`app/(dashboard)/layout.tsx`)**: 3 arreglos aplicados — (a) bug real: el link de Toolkit/Business Reports nunca se marcaba activo en sus subpáginas (`/toolkit/seo-audit` etc.), solo en la raíz exacta, a diferencia de sus hermanos Questionnaires/Documents que ya usaban `.startsWith(...)` — corregido con la misma condición; (b) el botón "Tour" no tenía feedback de fondo al hover, a diferencia de sus vecinos Project Memory/Resources — añadido `hover:bg-surface`; (c) 5 etiquetas en español dentro de una lista por lo demás en inglés (Cuestionarios/Documentos/Términos/Privacidad + 2 `aria-label`) — pasadas a inglés, consistente con la regla del proyecto de que la UI del portal MIRA es 100% en inglés.

**Pendiente real que queda, no arreglado hoy (fuera de alcance de este lote)**:
- Los labels del sidebar ahora en inglés (Questionnaires/Terms/Privacy) llevan a páginas de destino que siguen 100% en español (`app/(dashboard)/questionnaires/page.tsx`, `app/terms/page.tsx`, `app/privacy/page.tsx`, incluyendo `<title>`) — inconsistencia real de idioma descubierta durante la revisión adversarial, no introducida hoy. Traducir esas 3 páginas es una tarea de i18n aparte, no un fix quirúrgico de esta ronda.
- Varios links del sidebar usan `hover:bg-{color}-500/8`, un modificador de opacidad que Tailwind nunca compila en este proyecto (solo `/15` y `/25` existen realmente) — el hover de esos links cambia el color de texto pero nunca muestra el tinte de fondo, en ambos temas. Preexistente, cosmético, no arreglado.
- Legibilidad de los badges de paso (círculos numerados) de `ArchitectArchetype.tsx` en modo claro con datos reales no se pudo verificar en vivo (no había ningún ejemplo con contenido generado en los datos de prueba) — revisado el código y el contraste calculado matemáticamente da bien (≥3:1 con los 10 colores reales de agente), pero queda sin confirmación visual real.
- El mecanismo de selectores de atributo para inline styles (`[style*="..."]`) que patchea `agent-pipeline-header.tsx` y los componentes "Result" de Toolkit se mantiene intacto — es una decisión de arquitectura, no debt, pero sigue siendo la superficie más frágil del sistema de temas (depende de que el navegador serialice los estilos inline exactamente como se espera).

---

## zz) ✅ Migración completa de mira/portal a `@sf/supabase` (2026-07-31)

Último ítem pendiente del plan de stack (entrada xx, Fase C): los 33 ficheros de `mira/portal` que aún construían su propio cliente Supabase directamente. Ejecutado con 32 agentes en paralelo (uno por fichero, categorizados de antemano en 3 patrones: service-role/`createClient` crudo, Server Component o Route Handler con `createServerClient`+cookies, cliente de navegador con `createBrowserClient`) + `proxy.ts` migrado a mano por separado (mayor riesgo — es el middleware que gatea el auth de toda la app) + revisión adversarial de seguridad + verificación en vivo de auth/sesión.

**Bug crítico encontrado por la revisión adversarial y arreglado — no llegó a producción**: mi primera migración de `proxy.ts` a la factory compartida (`createServerComponentClient`) introducía una regresión real de sesión. La factory llama a su callback de cookies **una vez por cookie**; el código original de `proxy.ts` necesita recrear su `response` **una vez por lote completo** (`@supabase/ssr` puede escribir varias cookies en una sola llamada cuando la sesión se fragmenta en `nombre.0`/`nombre.1`/... por tamaño — plausible para super_admins con `user_metadata` más pesado). Con la factory, cada cookie nueva recreaba `response` desde cero, descartando el `Set-Cookie` de la cookie anterior — solo la última cookie del lote habría llegado al navegador (típico patrón de "cierres de sesión aleatorios" que la propia documentación de `@supabase/ssr` advierte). Arreglado revirtiendo `proxy.ts` a usar `createServerClient` de `@supabase/ssr` directamente, con el `setAll(cookiesToSet)` real preservando la semántica de lote-completo del código original — es la única excepción deliberada a "todo pasa por `@sf/supabase`", documentada con un comentario explicando por qué. Verificado en vivo forzando un refresh de sesión real (token expirado + refresh_token válido): `Set-Cookie` único y bien formado, `/admin` responde 200 tras el refresh.

**Otro hallazgo real, no arreglado (fuera de alcance)**: `app/api/load-missing-pillars/route.ts` tiene una rama muerta — cuando la service role key SÍ está configurada, la función nunca construye ningún cliente y cae directo a un 400 "Service key required"; el único `createClient` real que se ejecuta en la práctica usa la ANON key (rama de fallback), pese a llamarse `createAdminClient`. Migrado solo el import (de `@supabase/supabase-js` a `@sf/supabase`, mismo `createClient` re-exportado) sin tocar la lógica rota — arreglarla es una tarea aparte, no de wiring de cliente.
...
**3 bugs reales encontrados en vivo, todos preexistentes (no introducidos por el trabajo de hoy) — ✅ los 3 arreglados y reverificados el mismo día**:
1. ✅ **`/home` no sobrevivía un hard reload para super_admin con cliente activo** (`app/(dashboard)/home/page.tsx`): un `useEffect` comprobaba `activeClient` antes de que el fetch async de `ClientProvider` lo resolviera, así que en cada recarga dura rebotaba a `/admin` en vez de mostrar el home del cliente activo. **Arreglado**: `ClientContext` (`lib/client-context.tsx`) ahora expone un flag `loading` (true hasta que el fetch de `/api/me/clients` resuelve, en un `finally`), y `home/page.tsx` espera a que sea `false` antes de decidir si redirige. Verificado en vivo: 3 hard reloads consecutivos sobre `/home` con cliente activo, ninguno rebotó a `/admin`.
2. ✅ **Archivar un proyecto no persistía de verdad** (`lib/hooks/useProjectManagement.ts`): el `.update().select().single()` desde el navegador devolvía 406, porque `mira_projects` tiene RLS activada (migración 0037) con **solo política de SELECT** — sin política de UPDATE/DELETE, cualquier update/delete con la anon key afecta 0 filas en silencio. `projects/[slug]/page.tsx` ignoraba el error y forzaba el estado local igualmente. **Arreglado**: nueva ruta server-side `PATCH`/`DELETE /api/projects/[id]` (`app/api/projects/[id]/route.ts`) que verifica acceso al `client_id` real del proyecto (resuelto desde la DB por el id de la URL, nunca del body) vía `userCanAccessClient` y muta con el cliente admin (bypassa RLS) — mismo patrón ya usado por la creación de proyectos. `updateProject`/`deleteProject` del hook ahora llaman a esta ruta en vez de Supabase directo. Revisión adversarial de seguridad completa: sin forma de spoofear `client_id` por body, sin forma de reasignar un proyecto a otro cliente, el chequeo de acceso es la primera sentencia en ambos handlers, `super_admin` no es auto-escalable. Único hallazgo menor (no de seguridad): `status` no tenía allowlist — añadido (`'active' | 'archived'`). Verificado en vivo: PATCH devuelve 200 con el proyecto real, 2 hard reloads seguidos mantienen "Archivado".
3. ✅ **Las llamadas `/api/*` sin sesión devolvían redirect 307 a `/login` en vez de 401** (`proxy.ts`): rompía el patrón habitual `res.ok ? res.json() : ...` en clientes que esperan JSON (confirmado disparándose de forma natural en `lib/client-context.tsx`). **Arreglado**: `proxy.ts` ahora devuelve `NextResponse.json({error:'unauthorized'}, {status:401})` cuando `pathname` empieza por `/api/` y no hay usuario; las páginas normales siguen redirigiendo a `/login` igual que antes. Verificado en vivo con `redirect:'manual'` (401 real, no 307) y con fetch por defecto (`res.ok=false`, body JSON parseable).

**Verificación conjunta de los 3 fixes**: `tsc --noEmit` + `next build` limpios. En vivo: los 3 escenarios reproducidos y confirmados arreglados (arriba), regresión completa sin issues nuevos (login, `/roster`, chat real con streaming grounded en Brand Brain real, cero errores de consola).

---

## bbb) Ronda de cierre del ecosistema 2026-08-03: pixels vía SF-CMS, SSL caído, 2 pipelines de deploy rotos, y el P0/P1 del backlog

Sesión "no pares hasta terminar todo" pedida por el CEO. Todo comiteado (`5bd8543`..`47791f3` + docs), desplegado y verificado en vivo.

**1. Sistema de pixels site-wide en SF-CMS + Salsa Burgers (petición directa del CEO)**. Hallazgo que cambió el diagnóstico: el CEO creía que "funcionan todos menos TikTok" — en realidad en salsaburgers.com solo disparaba el Meta Pixel (env var). La cadena estaba rota en 3 puntos: `projects.settings` del CMS vacío (`{}`), `CMS_API_URL` VACÍA en el Vercel de Salsa (el build se saltaba el fetch entero), y el fetch sobreescribía `settings.json` con nulls al fallar (apagón silencioso de pixels). Construido: settings de proyecto extendidos al set completo (GA4/GTM/Meta/Ads+label/TikTok/LinkedIn) en `/api/public/settings` (aditivo), PATCH admin con merge server-side de jsonb, nueva UI `/admin/projects/[id]/pixels` (campos compartidos en `lib/pixels.ts::PROJECT_PIXEL_FIELDS`), IDs reales de Salsa guardados en BD (update scoped por id), envs repuestas (Prod+Preview, el add de preview vía API por el bug conocido del CLI), layout de Salsa con TikTok+Ads+Meta-desde-CMS (fallback env conservado), y fetch resiliente (conserva el settings.json anterior si el fetch falla). El contenedor GTM-5ZCM7FBJ está VACÍO (verificado descargándolo) → GA4/Ads directos sin riesgo de duplicado; si algún día se montan tags dentro de GTM, quitar los directos (avisado en el help de la UI). **Verificado en vivo: los 5 pixels en el HTML de producción de salsaburgers.com.**

**2. SSL de startupsfactory.es caído (reporte del CEO "aparece como no segura")**: el cert de `www` caducó el 31-jul (el apex estaba bien) — la auto-renovación de Vercel falló solo para www. Renovado vía `vercel certs issue` sin tocar la asignación de dominios (regla post-C3PO). De paso: `mira.startupsfactory.es` caducaba en 4 días con la renovación igualmente atascada (86d de edad) — renovado proactivo. Ambos verificados sirviendo con `notAfter` nov-2026.

**3. Dos pipelines de deploy rotos descubiertos al desplegar (ninguno se conocía)**: `sf-cms` y `sf-crm` llevaban desde la adopción de `@sf/supabase` (workspace:*) SIN PODER DESPLEGAR — sus proyectos Vercel no eran monorepo-aware (sin rootDirectory, sin GitHub, npm aislado → EUNSUPPORTEDPROTOCOL). Producción seguía sirviendo código pre-migración. Arreglado con el patrón probado de mira-portal: rootDirectory + link GitHub + framework nextjs (sf-crm tenía además el bug conocido `framework: services` del vercel link, Y un vercel.json que hardcodeaba `npm install`, Y las envs de Supabase VACÍAS en Vercel — repuestas con las keys vigentes, Y el filtro estricto de turbo que no declaraba `SUPABASE_SERVICE_KEY` en globalEnv — 4 capas de cebolla, todas documentadas en commits). Ambos `READY` y verificados en vivo.

**4. P0 del backlog del ecosistema (docs/audits/ECOSISTEMA-BUGS-2026-08-03.md) — arreglado y en producción**: `<html>`/`<body>` duplicados de startupsfactory.es (el [locale] layout es ahora el único root, lang dinámico conservado, 12 stub pages muertas fuera — verificado en vivo: 1 html/1 body/lang correcto); newsletter fake de NC Global (ahora envía de verdad vía formsubmit AJAX con error visible); links 404 del hub sf-reports (desactivados como el resto de formatos no disponibles); sf-crm leads mapeados (patrón crm_contacts) + rutas de schema ausente degradando con elegancia (el DDL real queda para decisión — 03_sf-crm-schema.sql NO es aplicable tal cual, drift severo documentado en el audit doc, y su GRANT afectaría a las tablas de MIRA en el proyecto compartido).
