...
**Pendiente real que queda**:
- Migrar los 33+ ficheros restantes de `mira/portal` que aún construyen su propio cliente Supabase directamente, si se decide que vale la pena (mayor esfuerzo/riesgo que lo ya hecho).
- `ai-agency-sf-next` todavía tiene `@ai-sdk/anthropic`/`ai` sin fijar (`"latest"`) — no se tocó, fuera del alcance de este lote (solo se fijaron las dependencias ya identificadas en el plan original).
