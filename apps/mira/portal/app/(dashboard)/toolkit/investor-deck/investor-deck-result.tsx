...
export function InvestorDeckResult({ data }: { data?: any }) {
  ...
  return (
    <div className="w-full bg-page">
      ...
      {/* Slides Navigation */}
      <div className="border-b border-line-subtle p-6 flex items-center justify-between backdrop-blur-sm">
        <button
            ...
            <div className="space-y-2">
              {data.team.slice(0, 3).map((member: any, idx: number) => (
                <div key={idx} className="border border-line bg-surface p-3 rounded">
                  ...
                  <div className="text-xs text-ink-tertiary mt-1">{member.background}</div>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="space-y-2">
              {data.board_and_advisors.map((advisor: any, idx: number) => (
                <div key={idx} className="text-ink-secondary">
                  ...
                  <div className="text-xs text-ink-tertiary">{advisor.background}</div>
                </div>
              ))}
            </div>
          </section>
