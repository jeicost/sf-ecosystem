  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-page border-b border-line p-6 md:p-8">
        <h1 className="text-5xl font-black text-ink mb-2">CONTENT PACK</h1>
        <p className="text-ink-secondary max-w-2xl">Complete content strategy: pillars, blog, social, email, video, calendar, and distribution plan</p>
      </div>
      ...
      <div className="bg-page p-6 md:p-8 space-y-8">
        ...
        {data.content_pillars && data.content_pillars.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Content Pillars</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {data.content_pillars.map((pillar: any, idx: number) => (
                <div key={idx} className="border-l-4 bg-surface p-4 rounded-r" style={{borderColor: '#ffd740'}}>
        ...
        {data.blog_content_hub && data.blog_content_hub.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Blog Content Hub</h2>
            <div className="space-y-3">
              {data.blog_content_hub.map((post: any, idx: number) => (
                <div key={idx} className="border border-line bg-surface p-4 rounded">
                  <div className="font-bold text-ink mb-1">{idx + 1}. {post.title}</div>
                  {post.outline && (
                  ...
                  {post.target_audience && <div className="text-xs text-ink-tertiary mt-2">Audience: {post.target_audience}</div>}
                  {post.word_count && <div className="text-xs text-ink-tertiary">Length: {post.word_count}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        {data.social_media_strategy && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Social Media Strategy</h2>
            <div className="space-y-4">
              {['instagram', 'tiktok', 'linkedin'].map((platform: string) => (
                data.social_media_strategy[platform] && (
                  <div key={platform} className="border border-line bg-surface p-4 rounded">
                    <div className="text-xs text-ink-secondary font-bold mb-2 uppercase">{platform}</div>
                    <div className="space-y-2">
                      {data.social_media_strategy[platform].map((item: any, idx: number) => (
                        <div key={idx} className="text-xs text-ink-secondary border-l border-line pl-2">
        ...
        {data.email_sequences && data.email_sequences.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Email Sequences</h2>
            <div className="space-y-3">
              {data.email_sequences.map((seq: any, idx: number) => (
                <div key={idx} className="border border-line bg-surface p-4 rounded">
                  <div className="font-bold text-ink mb-2">{seq.name || `Sequence ${idx + 1}`}</div>
                  <div className="text-xs text-ink-secondary mb-2">Subject: {seq.subject}</div>
                  {seq.body_outline && (
                  ...
                  {seq.send_timing && <div className="text-xs text-ink-tertiary">Timing: {seq.send_timing}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        {data.video_content_briefs && data.video_content_briefs.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Video Content Briefs</h2>
            <div className="space-y-3">
              {data.video_content_briefs.map((video: any, idx: number) => (
                <div key={idx} className="border border-line bg-surface p-4 rounded">
                  <div className="font-bold text-ink mb-2">{video.type}: {video.title || `Video ${idx + 1}`}</div>
                  {video.script_outline && (
                    <div className="text-xs text-ink-secondary mb-2">Script: {video.script_outline.slice(0, 150)}...</div>
                  )}
                  {video.visuals && <div className="text-xs text-ink-tertiary">Visuals: {video.visuals}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        {data.content_repurposing && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Content Repurposing Framework</h2>
            <div className="border border-line bg-surface p-4 rounded">
              {data.content_repurposing.blog_post_to_5_formats && (
                <div>
                  <div className="text-xs text-ink-secondary font-bold mb-2">1 BLOG POST → 5 FORMATS</div>
        ...
        {data.distribution_amplification && data.distribution_amplification.channels && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Distribution & Amplification</h2>
            <div className="space-y-3">
              {data.distribution_amplification.channels.map((ch: any, idx: number) => (
                <div key={idx} className="border border-line bg-surface p-4 rounded">
                  <div className="font-bold text-ink mb-2">{ch.platform}</div>
                  <div className="text-xs text-ink-secondary mb-1">Cadence: {ch.cadence}</div>
                  {ch.tactics && (
                    <div className="text-xs text-ink-tertiary">
        ...
        {data.content_calendar && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">12-Month Content Calendar</h2>
            <div className="border border-line bg-surface p-4 rounded max-h-96 overflow-y-auto">
              <div className="text-xs text-ink-secondary">
        ...
        {data.analytics_measurement && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Analytics & Measurement</h2>
            <div className="space-y-3">
              {data.analytics_measurement.kpis_per_type && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">KPIS PER CONTENT TYPE</div>
                  <div className="text-xs text-ink-secondary">
              ...
              {data.analytics_measurement.dashboards && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">DASHBOARDS</div>
                  <div className="text-xs text-ink-secondary">{data.analytics_measurement.dashboards}</div>
                </div>
              )}
              {data.analytics_measurement.cadence && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">REPORTING CADENCE</div>
                  <div className="text-xs text-ink-secondary">{data.analytics_measurement.cadence}</div>
                </div>
              )}
            </div>
          </section>
        ...
        {data.ugc_strategy && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">User-Generated Content Strategy</h2>
            <div className="space-y-3">
              {data.ugc_strategy.hashtags && data.ugc_strategy.hashtags.length > 0 && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">CAMPAIGN HASHTAGS</div>
              ...
              {data.ugc_strategy.testimonial_program && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">TESTIMONIAL PROGRAM</div>
                  <div className="text-xs text-ink-secondary">{data.ugc_strategy.testimonial_program}</div>
                </div>
              )}
              {data.ugc_strategy.community_content && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">COMMUNITY CONTENT</div>
                  <div className="text-xs text-ink-secondary">{data.ugc_strategy.community_content}</div>
                </div>
              )}
            </div>
          </section>
