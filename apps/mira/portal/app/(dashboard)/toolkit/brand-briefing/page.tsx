...
const TOOL_CONFIG: ToolConfig = {
  ...
  timing: '20-30 min',
  brandBrainNote: 'Misión, audiencia, personalidad y colores entran solos desde el Brand Brain',
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-page border-b border-line p-6 md:p-8">
        <h1 className="text-5xl font-black text-ink mb-2">BRAND BRIEFING</h1>
        <p className="text-ink-secondary max-w-2xl">Complete brand strategy: identity, positioning, audience, pillars, voice, visual, content, and evolution roadmap</p>
      </div>
      ...
      <div className="bg-page p-6 md:p-8 space-y-8">
        ...
        {data?.brand_identity && (
          <section>
            ...
            {data.brand_identity.values && data.brand_identity.values.length > 0 && (
              <div className="mt-4">
                <div className="text-xs text-ink-secondary mb-2 font-bold">VALUES</div>
        ...
        {data?.brand_promise && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Brand Promise</h2>
        ...
        {data?.competitive_positioning && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Competitive Positioning</h2>
            <div className="space-y-3">
        ...
        {data?.target_audience && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Target Audience</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.target_audience.personas.map((persona: any, idx: number) => (
                  <div key={idx} className="border border-line bg-surface p-4 rounded">
                    ...
                    {persona.pain_points && (
                      <div className="mt-2">
                        <div className="text-xs text-ink-tertiary mb-1">Pain points:</div>
        ...
        {data?.brand_pillars && data.brand_pillars.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Brand Pillars</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {data?.brand_voice && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Brand Voice</h2>
            <div className="space-y-4">
              {data.brand_voice.tone && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">TONE</div>
                  <p className="text-sm text-ink-secondary">{data.brand_voice.tone}</p>
                </div>
              )}
              {data.brand_voice.traits && data.brand_voice.traits.length > 0 && (
                <div>
                  <div className="text-xs text-ink-secondary font-bold mb-2">PERSONALITY TRAITS</div>
                  <div className="flex flex-wrap gap-2">
        ...
        {data?.visual_identity && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Visual Identity</h2>
            {data.visual_identity.colors && data.visual_identity.colors.length > 0 && (
              <div className="mb-4">
                <div className="text-xs text-ink-secondary font-bold mb-3">COLOR PALETTE</div>
        ...
        {data?.brand_values_in_practice && data.brand_values_in_practice.length > 0 && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Values in Practice</h2>
            <div className="space-y-3">
        ...
        {data?.customer_journey_touchpoints && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Customer Journey Touchpoints</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {['awareness', 'consideration', 'decision', 'loyalty'].map((stage: string) => (
                data.customer_journey_touchpoints[stage] && (
                  <div key={stage} className="border border-line bg-surface p-4 rounded">
                    <div className="text-xs text-ink-secondary font-bold mb-2 uppercase">{stage}</div>
        ...
        {data?.brand_evolution && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Brand Evolution Strategy</h2>
            <div className="space-y-3">
              {data.brand_evolution['2_year_roadmap'] && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">2-YEAR ROADMAP</div>
              ...
              {data.brand_evolution.potential_expansions && data.brand_evolution.potential_expansions.length > 0 && (
                <div className="border border-line bg-surface p-4 rounded">
                  <div className="text-xs text-ink-secondary font-bold mb-2">POTENTIAL EXPANSIONS</div>
                  <ul className="text-xs text-ink-secondary space-y-1">
        ...
        {data?.success_metrics && (
          <section>
            <h2 className="text-2xl font-black text-ink uppercase mb-4">Success Metrics</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.success_metrics.kpis.map((kpi: any, idx: number) => (
                  <div key={idx} className="border border-line bg-surface p-4 rounded">
                    ...
                    <div className="text-xs text-ink-secondary">Tracking: {kpi.tracking}</div>
                  </div>
                ))}
              </div>
            )}
