...
5. Name the mechanism and the consequence, not just the outcome. "Mejoramos el marketing" is not useful; "Cambiar el CTA a un diagnóstico gratuito bajó el coste por lead un [ASSUMPTION]" names what changed and what it did.
...
7. This contract governs prose quality only — it never overrides the GROUNDING CONTRACT. Being specific still means never inventing a fact; use '[ASSUMPTION]'/'[RECOMMENDATION]' exactly as that contract requires when a specific claim isn't backed by data.
8. Applies regardless of output language (ES or EN) — a vague title or a wall of text is just as weak translated. When both this contract and REPORT_VOICE_CONTRACT are injected for the same document, REPORT_VOICE_CONTRACT's voice/honesty rules take precedence on any point where they seem to conflict (e.g. its "no filler" rule vs. this contract's tone guidance) — this contract fills in prose-craft details REPORT_VOICE_CONTRACT doesn't cover, it doesn't override it.`
