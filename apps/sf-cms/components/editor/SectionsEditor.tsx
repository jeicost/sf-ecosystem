    ...
    <div>
      ...
      <div className="space-y-4">
        {sections.map((section, i) => (
          <div key={section.id || i} className="overflow-hidden rounded-lg border border-slate-200 bg-white">
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-3 py-2">
              <div className="flex items-center gap-2">
                <span className="rounded bg-slate-200 px-2 py-0.5 font-mono text-[0.62rem] uppercase tracking-wide text-slate-600">
                  {section.type}
                </span>
                <span className="font-mono text-[0.68rem] text-slate-400">{section.id}</span>
              </div>
              <div className="flex items-center gap-0.5 text-slate-400">
                <button onClick={() => move(i, -1)} disabled={i === 0} className="p-1 hover:text-slate-800 disabled:opacity-30" title="Subir"><ChevronUp size={15} /></button>
                <button onClick={() => move(i, 1)} disabled={i === sections.length - 1} className="p-1 hover:text-slate-800 disabled:opacity-30" title="Bajar"><ChevronDown size={15} /></button>
                <button onClick={() => duplicate(i)} className="p-1 hover:text-slate-800" title="Duplicar"><Copy size={14} /></button>
                <button onClick={() => setJsonOpenIdx(jsonOpenIdx === i ? null : i)} className={`p-1 hover:text-slate-800 ${jsonOpenIdx === i ? 'text-slate-800' : ''}`} title="JSON"><Code2 size={14} /></button>
                <button onClick={() => remove(i)} className="p-1 text-red-400 hover:text-red-600" title="Eliminar"><Trash2 size={14} /></button>
              </div>
            </div>
      ...
      {/* add section */}
      <div className="mt-4 flex items-center gap-2 rounded-lg border border-dashed border-slate-300 p-2.5">
        <select
          value={newType}
  ...
  return (
    <div>
      <textarea
        ...
        rows={12}
        className="w-full rounded-md border border-slate-300 p-2.5 font-mono text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400"
      />
