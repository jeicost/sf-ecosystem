...
export function MapSection({ content }: { content: HomeContent }) {
  const pills = [1, 2, 3, 4, 5].map((n) => ({
    cat: content[`map_pin_${n}_cat` as keyof HomeContent],
    name: content[`map_pin_${n}_name` as keyof HomeContent],
  }));
  ...
  return (
    <section className="section" id="mapa" style={{ background: "var(--bg-soft)" }} aria-labelledby="map-title">
      <div className="container">
        ...
        <Reveal delay={100}>
          <div className="map" role="application" aria-label="Mapa interactivo de la ciudad">
            <aside className="map__sidebar" aria-label="Puntos de interés cercanos">
